...
function connectToChannelAuth(apiKey, apiSecret, channelName) {
...
/**
 * Main library wrapper for Bitflyer Websocket API.
 *
...
module.exports = function BitflyerWs(config) {
  ...
  const bitflyerWsConnection = {
    ...
    onOpen: function (onOpenFunction) {
      ...
      if (this.ws) {
        this.ws.on('connect', onOpenFunction);
      }
    },
    ...
    removeOnOpen: function (removeOnOpenFunction) {
      ...
      if (this.ws) {
        this.ws.removeEventListener('connect', removeOnOpenFunction);
      }
    },
    ...
    onClose: function (onCloseFunction) {
      ...
      if (this.ws) {
        this.ws.on('disconnect', onCloseFunction);
      }
    },
    ...
    removeOnClose: function (removeOnCloseFunction) {
      ...
      if (this.ws) {
        this.ws.removeEventListener('disconnect', removeOnCloseFunction);
      }
    },
    ...
    onMessage: function (onMessageFunction) {
      ...
      if (this.ws) {
        this.ws.on(channelName, onMessageFunction);
      }
    },
    ...
    removeOnMessage: function (removeOnMessageFunction) {
      ...
      if (this.ws) {
        this.ws.removeEventListener(channelName, removeOnMessageFunction);
      }
    },
    ...
    disconnect: function () {
      onOpenFunctions.forEach(
        onOpenFunction => this.ws
          .removeEventListener('connect', onOpenFunction),
      );
      onCloseFunctions.forEach(
        onCloseFunction => this.ws
          .removeEventListener('disconnect', onCloseFunction),
      );
      onErrorFunctions.forEach(
      ...
      onMessageFunctions.forEach(
        onMessageFunction => this.ws
          .removeEventListener(channelName, onMessageFunction),
      );
  ...
  if (apiKey && apiSecret) {
    onOpenFunctions.push(connectToChannelAuth.bind(bitflyerWsConnection,

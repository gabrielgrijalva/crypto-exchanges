...
const BASE_URL = 'wss://ws.bitstamp.net';

/**
 * Checker process that validates that bitstampWsConnection is open through ping/pong implementation.
 *
 */
...
/**
 * Main library wrapper for Bitstamp Websocket API.
 *
 * @param {Object} config Configuration object for creating a new websocket connection.
 */
module.exports = function BitstampWs(config) {
  ...
  onOpenFunctions.push(connectionChecker.bind(bitstampWsConnection));
  onOpenFunctions.push(() => bitstampWsConnection.send(JSON.stringify(
    request)));

  return bitstampWsConnection;
};

 ...
 * @param {string} apiSubaccount Secure user account api subaccount.
 *
 * @return {Function} Promise that is executed to make a private call to the API.
 */
module.exports = function privateCall(baseUrl, apiKey, apiSecret, apiSubaccount) {
  return (async function (path, method, queryParams, bodyParams) {
    ...
    if (apiSubaccount) {
      requestOptions.headers['FTX-SUBACCOUNT'] = apiSubaccount;
    }

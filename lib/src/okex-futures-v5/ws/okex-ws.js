...
const BASE_URL = 'wss://ws.okex.com:8443/ws/v5';
...
module.exports = function OkexWs(config) {
  ...
  function onOpenDefault() {
    if (channelType === 'private') {
      ...
      const request = { op: 'subscribe', args: [{ channel: channelName, instType: 'FUTURES', instId: symbol, }], };
  ...
  function onMessageDefault(message) {
    ...
    if (messageParsed.event === 'login' && messageParsed.code === '0') {
      const subscription = JSON.stringify({
        op: 'subscribe',
        args: [[{ channel: channelName, instType: 'FUTURES', instId: symbol, }]],
      });
   ...
   */
     ...
     */
    connect: function () {
      this.ws = new WebSocket(`${BASE_URL}/${channelType}`);

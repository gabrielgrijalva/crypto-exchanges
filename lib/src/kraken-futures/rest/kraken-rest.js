...
const PrivateCallBatch = require('./calls/private-call-batch');

const BASE_URL = 'https://api.futures.kraken.com/derivatives';
const BASE_URL_GLOBAL = 'https://futures.kraken.com/derivatives';

module.exports = function BinanceFuturesRest(apiKey, apiSecret) {
  const client = {};
  const publicCall = PublicCall(BASE_URL);
  const publiCallGlobal = PublicCall(BASE_URL_GLOBAL);
  const privateCall = PrivateCall(BASE_URL, apiKey, apiSecret);
  const privateCallBatch = PrivateCallBatch(BASE_URL, apiKey, apiSecret);
  // PUBLIC ENDPOINTS
  ...
  client.getChartsTrade = function (params) {
    return publiCallGlobal(`/api/v4/charts/trade/${params
  ...
  client.postBatchOrder = function (params) {
    return privateCallBatch('/api/v3/batchorder', 'POST', params);
  };

...
const BASE_URL = 'wss://stream.bybit.com/realtime';

function getAuthentication(apiKey, apiSecret) {
  const expires = Date.now() + 5000;
  const signature = crypto.createHmac('sha256', apiSecret).update(
    `GET/realtime${expires}`).digest('hex');
  return [apiKey, expires, signature];
};

/**
 * Main library wrapper for Bybit Websocket API.
 *
 * @param {Object} config Configuration object for creating a new websocket connection.
 */
module.exports = function BybitWs(config) {
  ...
  onOpenFunctions.push(function defaultOnOpen() {
    const request = {};
    request.op = 'auth';
    request.args = getAuthentication(apiKey, apiSecret);
    bybitWsConnection.send(JSON.stringify(request));
  });
  onOpenFunctions.push(function connectionChecker() {
    let sendPingTimeout = 0;
    let closeConnectionTimeout = 0;
    const closeConnection = () => {
      if (bybitWsConnection.ws && bybitWsConnection.ws
        .readyState === bybitWsConnection.ws.OPEN) {
        bybitWsConnection.ws.close();
      }
    };
    const sendPing = () => {
      if (!bybitWsConnection.ws || bybitWsConnection.ws
    ...
    bybitWsConnection.ws.on('pong', pongCallback);
    bybitWsConnection.ws.ping();
  });
  onMessageFunctions.push(function defaultOnMessage(message) {
    const messageParsed = JSON.parse(message);
    if (messageParsed.success && messageParsed.request.op === 'auth') {
      bybitWsConnection.send(JSON.stringify(config));
    }
  });
  return bybitWsConnection;
};

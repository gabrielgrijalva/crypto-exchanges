...
const BinanceRest = require('../rest/binance-rest');
...
module.exports = function BinanceWs(config) {
  ...
  const clientRest = stream === 'user' ? BinanceRest(config.apiKey,
  ...
  const binanceWsConnection = {
    ...
    connect: async function () {
      const connectionURL = `${BASE_URL}/ws/${stream !== 'user'
        ? stream : (await clientRest.postUserDataStream()).listenKey}`;

      this.ws = new WebSocket(connectionURL);
      this.ws.on('ping', () => this.ws.pong());
      clearInterval(listenKeyIntervalId);
      listenKeyIntervalId = setInterval(() => clientRest ? clientRest
        .postUserDataStream() : null, 1800000);

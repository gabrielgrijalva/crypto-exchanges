...
const BASE_URL = 'wss://ws-feed.pro.coinbase.com';

/**
 * Checker process that validates that coinbaseWsConnection is open through ping/pong implementation.
 *
 */
...
/**
 * Main library wrapper for Coinbase Websocket API.
 *
 * @param {Object} config Configuration object for creating a new websocket connection.
 */
module.exports = function CoinbaseWs(config) {
  ...
  onOpenFunctions.push(connectionChecker.bind(coinbaseWsConnection));
  onOpenFunctions.push(() => coinbaseWsConnection.send(JSON.stringify(
    request)));

  return coinbaseWsConnection;
};
